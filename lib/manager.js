...
Manager.prototype.onConnectToPeer = function(message) {
  ...
  peer.on('connect', function(peer) {
    console.log('connect with %s', username);
    
    peer.on('message', function(message) {
      console.log('peer message from %s: %s', username, util.inspect(message, false, 4, true));
    });
  ...
  peer.on('error', function() {
    console.log('connect error with ' + username, arguments);
  });
};
